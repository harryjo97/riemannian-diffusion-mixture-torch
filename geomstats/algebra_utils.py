...
# taylor_exp_even_func, flip_determinant
# for gpu device
...
def taylor_exp_even_func(point, taylor_function, order=5, tol=EPSILON):
    ...
    point_ = gs.where(gs.abs(point) <= tol, tol * gs.ones_like(point), point)
...
def flip_determinant(matrix, det):
    ...
    """
    ones = torch.ones(matrix.shape[-1], device=matrix.device)
    # reflection_vec = gs.concatenate([ones[:-1], gs.array([-1.0])], axis=0)
    reflection_vec = torch.cat([ones[:-1], -ones[-1].unsqueeze(-1)], axis=0)
