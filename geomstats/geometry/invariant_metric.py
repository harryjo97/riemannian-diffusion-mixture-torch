...
class BiInvariantMetric(_InvariantMetricVector):
    ...
    def inner_product_at_identity(self, tangent_vec_a, tangent_vec_b):
        ...
        inner_prod : array-like, shape=[...]
            Inner-product of the two tangent vectors.
        """
        shape = tangent_vec_a.shape
        dim = self.group.n
        tangent_vec_a = tangent_vec_a.reshape(-1, dim, dim)
        tangent_vec_b = tangent_vec_b.reshape(-1, dim, dim)
    ...
    #NOTE: Used for SO3 Wrapped Normal log prob in distribution.py
    def transpback0(self, x, v):
        shape = v.shape
        dim = self.group.n
        x = x.reshape(-1, dim, dim)
        v = v.reshape(-1, dim, dim)
        mat_rep = self.group.compose(self.group.inverse(x), v) #.reshape(shape)
        return self.group.lie_algebra.basis_representation(mat_rep)